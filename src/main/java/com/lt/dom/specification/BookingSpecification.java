...
public class BookingSpecification implements Specification<Reservation> {

    private BookingQueryfieldsCriteria criteria;

    public BookingSpecification(BookingQueryfieldsCriteria searchCriteria) {
      ...
      (Root<Reservation> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
        ...
        if (!ObjectUtils.isEmpty(criteria.getStatuses())) {
            predicates.add(builder.in(
                    root.<List<EnumBookingStatus>> get("status")).value(criteria.getStatuses()));
        }
