...
@RestController
@RequestMapping("/oct/share")
public class ShareCommunityRestController {
    ...
    public EntityModel getInvoice(@PathVariable long INVOICE_ID) {
        ...
        entityModel.add(linkTo(methodOn(ShareCommunityRestController.class).listInvoices(subscription.getId(),null,null)).withRel("listItem"));
    ...
    public PagedModel listInvoices(@PathVariable long PRODUCT_ID,Pageable pageable, PagedResourcesAssembler<EntityModel<Subscription>> assembler) {
        ...
        return assembler.toModel(validatorOptional.map(e->{
            ...
            entityModel.add(linkTo(methodOn(ShareCommunityRestController.class).getInvoice(e.getId())).withSelfRel());
