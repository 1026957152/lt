...
import com.lt.dom.serviceOtc.Axh.XhSupplierServiceImp;
...
@RestController
@RequestMapping("/xh")
public class XhSupplierRestController {
    @Autowired
    ...
    @Autowired
    private XhSupplierServiceImp supplierService;
...
*/
     ...
     //   Page<Supplier> supplier = supplierRepository.findAll(example,paging);
        ...
        Page<EntityModel<SupplierResp>> page1 = supplier.map(x->{
            ...
            EntityModel entityModel = EntityModel.of(SupplierResp.from(x,assets));
            entityModel.add(linkTo(methodOn(XhSupplierRestController.class).deleteSupplier(x.getId())).withRel("delete"));
...
/*        ExampleMatcher modelMatcher = ExampleMatcher.matching()
        ...
        Example<Supplier> example = Example.of(probe, modelMatcher);*/
            Optional<Supplier> optionalSupplier = supplierRepository.findByName(pojo.getName());
...
/*
        ...
        if(supplier.isEmpty()) {
            ...
            entityModel.add(linkTo(methodOn(XhSupplierRestController.class).page_addEmployee(supplier.get().getId())).withRel("Page_addEmployees"));

            entityModel.add(linkTo(methodOn(XhSupplierRestController.class).getEmployeeList(supplier.get().getId(),null)).withRel("listEmployees"));
...
/*
        ...
        entityModel.add(linkTo(methodOn(XhSupplierRestController.class).createEmployee(supplier.getId(),null)).withRel("addEmployees"));
...
Employee employee = triplet.getValue1();
    ...
    @GetMapping(value = "/suppliers__/{SUPPLIER_ID}/employees", produces = "application/json")
